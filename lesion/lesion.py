...
def lesion_2020(gha_dict_path,
    ...
    if verbose:
        if test_run:
            ...
            #       f"type([0][0][0]): {type(x_data[0][0][0])}\n{x_data}")
            print(f"type(x_data[0][0][0][0]): {type(x_data[0][0][0][0])}")
    ...
    # not all network layers have associated weights (only learnable ones)
    ...
    # # 4. run get scores on ALL items - record total acc, class acc, item success
